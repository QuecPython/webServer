...
class WebTemplate :
	...
	def _parseBloc(self, execute) :
		while self._pos <= self._endPos :
			...
			if c == WebTemplate.TOKEN_OPEN[0] and \
			    self._code[ self._pos : self._pos + WebTemplate.TOKEN_OPEN_LEN ] == WebTemplate.TOKEN_OPEN :
				self._pos    += WebTemplate.TOKEN_OPEN_LEN
				tokenContent  = ''
				x 			  = self._pos
				while True :
					if x > self._endPos :
						raise Exception("%s is missing (line %s)" % (WebTemplate.TOKEN_CLOSE, self._line))
					c = self._code[x]
					if c == WebTemplate.TOKEN_CLOSE[0] and \
					   self._code[ x : x + WebTemplate.TOKEN_CLOSE_LEN ] == WebTemplate.TOKEN_CLOSE :
	...
	def _processInstructionINCLUDE(self, instructionBody, execute) :
		if not instructionBody :
			raise Exception( '"%s" alone is an incomplete syntax (line %s)' % (WebTemplate.INSTRUCTION_INCLUDE, self._line) )
